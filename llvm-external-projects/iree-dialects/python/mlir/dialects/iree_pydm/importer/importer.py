...
class FunctionDefBodyImporter(BaseNodeVisitor):
    ...
    if not self._last_was_return:
      with ic.ip, ic.loc:
        ...
        if self.successor_block:
          ...
          # TODO: Teach ODS how to build an op with successors.
          std_d.BranchOp.build_generic(results=[],
                                       operands=[],
                                       successors=[self.successor_block])
        else:
  ...
  def visit_Assign(self, node: ast.Assign):
    ...
    for target in node.targets:
      ...
      with ic.loc, ic.ip:
        d.StoreFreeVarOp(ir.StringAttr.get(target.id), boxed)
  ...
  def visit_If(self, node: ast.If):
    ...
    with ic.ip, ic.loc:
      # TODO: Teach ODS how to deal with successors.
      std_d.CondBranchOp.build_generic(results=[],
                                       operands=[test_pred, [], []],
...
class ExpressionImporter(BaseNodeVisitor):
  ...
  def get_call_result(self, args: Sequence[ir.Value]) -> ir.Value:
    ...
    if isinstance(self._result, ir.Value):
      return self.abort(f"TODO: User defined function call not supported")
    else:
      # Intrinsic.
...
def _get_function_ast(f) -> ast.AST:
