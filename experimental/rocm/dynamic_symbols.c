...
static iree_status_t iree_hal_rocm_dynamic_symbols_resolve_all(iree_hal_rocm_dynamic_symbols_t *syms) {
#define RC_PFN_DECL(rocmSymbolName, ...)                                                                \
  {                                                                                                     \
    static const char *kName = #rocmSymbolName;                                                         \
    IREE_RETURN_IF_ERROR(                                                                               \
      iree_dynamic_library_lookup_symbol(syms->loader_library, kName, (void **)&syms->rocmSymbolName)); \
  }
...
iree_status_t iree_hal_rocm_dynamic_symbols_initialize(iree_allocator_t allocator,
                                                       iree_hal_rocm_dynamic_symbols_t *out_syms) {
  IREE_TRACE_ZONE_BEGIN(z0);
  memset(out_syms, 0, sizeof(*out_syms));
  iree_status_t status =
    iree_dynamic_library_load_from_files(IREE_ARRAYSIZE(kROCMLoaderSearchNames), kROCMLoaderSearchNames,
...
void iree_hal_rocm_dynamic_symbols_deinitialize(iree_hal_rocm_dynamic_symbols_t *syms) {
  IREE_TRACE_ZONE_BEGIN(z0);
