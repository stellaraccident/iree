...
static iree_hal_rocm_semaphore_t *iree_hal_rocm_semaphore_cast(iree_hal_semaphore_t *base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_rocm_semaphore_vtable);
  return (iree_hal_rocm_semaphore_t *)base_value;
}

iree_status_t iree_hal_rocm_semaphore_create(iree_hal_rocm_context_wrapper_t *context, uint64_t initial_value,
  ...
  iree_status_t status = iree_allocator_malloc(context->host_allocator, sizeof(*semaphore), (void **)&semaphore);
  if (iree_status_is_ok(status)) {
    iree_hal_resource_initialize(&iree_hal_rocm_semaphore_vtable, &semaphore->resource);
...
static void iree_hal_rocm_semaphore_destroy(iree_hal_semaphore_t *base_semaphore) {
  iree_hal_rocm_semaphore_t *semaphore = iree_hal_rocm_semaphore_cast(base_semaphore);
...
static iree_status_t iree_hal_rocm_semaphore_query(iree_hal_semaphore_t *base_semaphore, uint64_t *out_value) {
  // TODO: Support semaphores completely.
  *out_value = 0;
  return iree_make_status(IREE_STATUS_UNIMPLEMENTED, "Not impemented on rocm");
}

static iree_status_t iree_hal_rocm_semaphore_signal(iree_hal_semaphore_t *base_semaphore, uint64_t new_value) {
...
static void iree_hal_rocm_semaphore_fail(iree_hal_semaphore_t *base_semaphore, iree_status_t status) {}

static iree_status_t iree_hal_rocm_semaphore_wait(iree_hal_semaphore_t *base_semaphore, uint64_t value,
