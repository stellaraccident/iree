...
RC_PFN_DECL(hipDeviceGetName, char *, int, hipDevice_t)  // No direct, need to modify
RC_PFN_STR_DECL(hipGetErrorName,
                hipError_t)  // Unlike other functions hipGetErrorName(hipError_t) return const char* instead of hipError_t so it uses a different macro
RC_PFN_STR_DECL(hipGetErrorString,
                hipError_t)  // Unlike other functions hipGetErrorName(hipError_t) return const char* instead of hipError_t so it uses a different macro
RC_PFN_DECL(hipInit, unsigned int)
RC_PFN_DECL(hipModuleLaunchKernel, hipFunction_t, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int,
...
RC_PFN_DECL(hipMemcpyAsync, void *, const void *, size_t, hipMemcpyKind, hipStream_t)
...
RC_PFN_DECL(hipModuleLoadDataEx, hipModule_t *, const void *, unsigned int, hipJitOption *, void **)
