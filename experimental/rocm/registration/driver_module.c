...
static iree_status_t iree_hal_rocm_driver_factory_enumerate(void *self, const iree_hal_driver_info_t **out_driver_infos,
...
static iree_status_t iree_hal_rocm_driver_factory_try_create(void *self, iree_hal_driver_id_t driver_id,
                                                             iree_allocator_t allocator,
                                                             iree_hal_driver_t **out_driver) {
  IREE_ASSERT_ARGUMENT(out_driver);
  *out_driver = NULL;
  if (driver_id != IREE_HAL_ROCM_DRIVER_ID) {
    return iree_make_status(IREE_STATUS_UNAVAILABLE, "no driver with ID %016" PRIu64 " is provided by this factory",
                            driver_id);
  }
  ...
  iree_status_t status = iree_hal_rocm_driver_create(identifier, &driver_options, allocator, out_driver);
